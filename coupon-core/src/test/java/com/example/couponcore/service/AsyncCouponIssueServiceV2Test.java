...
class AsyncCouponIssueServiceV2Test extends TestConfig {
    ...
    void issue_2(){
        ...
        CouponIssueException exception =
                Assertions.assertThrows(CouponIssueException.class, () -> sut.issue(coupon.getId(), userId));
        //then
    ...
    @Test
    @DisplayName("쿠폰 발급 - 발급기한이 유효하지 않다면 예외를 반환한다.")
    void issue_4(){
        //given
        long userId = 1;
        Coupon coupon = Coupon.builder()
    ...
    @Test
    @DisplayName("쿠폰 발급 - 발급 요청이 성공하면 쿠폰 발급 큐에 적재된다.")
    void issue_6() throws JsonProcessingException {
        ...
        Coupon coupon = Coupon.builder()
                ...
                .build();
        couponJpaRepository.save(coupon);
        CouponIssueRequest issueRequest = new CouponIssueRequest(coupon.getId(), userId);

        //when
        sut.issue(coupon.getId(),userId);
        //then
        String savedIssueRequest = redisTemplate.opsForList().leftPop(getIssueRequestQueueKey());
        Assertions.assertEquals(new ObjectMapper().writeValueAsString(issueRequest), savedIssueRequest);
    }
